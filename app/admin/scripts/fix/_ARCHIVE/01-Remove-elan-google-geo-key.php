...
/**
 * Remove Google Geocoding API Key Script
 *
 * Administrative script to remove the `elan_google_geo_key` column from the
 * `settings` table. This column stored the Google Geocoding API key which was
 * deprecated in v2.11.0 and removed in v2.22.0.
 * Issue #433: Remove Google Geocoding API Key
 *
 * Must run before the #724 fix script (which removes `elan_google_maps_key`).
 *
 * DEPLOYMENT INSTRUCTIONS:
 * 1. Script lives in app/admin/scripts/fix/ as a one-time migration: 01-Remove-elan-google-geo-key.php
 * 2. Scripts are accessed via manage-maintenance.php (Maintenance tab) or direct URL
 * 3. Sequential numbering (01) ensures it runs before later fix scripts
    ...
    <div class="container-fluid">
        <div class="well">
                ...
                <div class="col-lg-12 mb-4">
                    <div class="card registry-card">
                        <div class="card-header">
                            <h2 class="mb-0">
                                <i class="fa fa-key"></i> Remove Google Geocoding API Key
                            </h2>
                        </div>
                        <div class="card-body">
                            <p class="mb-3">Removes the deprecated <code>elan_google_geo_key</code> column from the <code>settings</code> table.</p>

                            <div class="alert alert-info">
                                <h5><i class="fa fa-info-circle"></i> What this script does:</h5>
                                <ul class="mb-0">
                                    <li>Drops the <code>elan_google_geo_key</code> column from the <code>settings</code> table</li>
                                    <li>Uses <code>DROP COLUMN IF EXISTS</code> for safe, idempotent execution</li>
                                    <li>Removes a setting deprecated in v2.11.0 and scheduled for removal in v2.22.0</li>
                                    <li>Logs completion to the <code>fix_script_runs</code> table</li>
                                    <li>Resolves GitHub issue #433</li>
                                </ul>
                            </div>
                            ...
                            <div class="alert alert-warning">
                                ...
                                <ul class="mb-0">
                                    <li>This script must run before the #724 fix script (which removes <code>elan_google_maps_key</code>)</li>
                                    <li>Backup the database before running destructive schema changes</li>
                                    <li>Any stored Google Geocoding API key value will be permanently lost when the column is dropped</li>
                                </ul>
                            </div>
            ...
            <div class="card registry-card">
                <div class="card-header">
                    <h2 class="mb-0">
                        <i class="fa fa-cogs"></i> Removing elan_google_geo_key Column
                    </h2>
                ...
                <div class="card-body">
                    ...
                    try {
                        // STEP 1: Drop the elan_google_geo_key column
                        logProgress(SECTION_SEPARATOR, 'step');
                        logProgress('STEP 1: Drop elan_google_geo_key column from settings table', 'step');
                        logProgress(SECTION_SEPARATOR, 'step');
                        ...
                        $exists = $db->query(
                            "SELECT COUNT(*) AS c FROM information_schema.COLUMNS
                             WHERE TABLE_SCHEMA = DATABASE()
                               ...
                               AND COLUMN_NAME = 'elan_google_geo_key'"
                        )->first();

                        if ((int)($exists->c ?? 0) === 0) {
                            logProgress('Column elan_google_geo_key does not exist; nothing to do.', 'info');
                        } else {
                            logProgress('Dropping elan_google_geo_key column...', 'info');
                            $db->query("ALTER TABLE settings DROP COLUMN elan_google_geo_key;");
                            ...
                            logProgress('Column elan_google_geo_key removed successfully.', 'success');
                        }
                        ...
                        $inserted = $db->insert('fix_script_runs', [
                            'script_name' => '01-Remove-elan-google-geo-key.php',
                        ...
                        if (!$inserted) {
                            ...
                            logger((int)$user->data()->id, LogCategories::LOG_CATEGORY_FIX_SCRIPT,
                                'Failed to insert fix_script_runs record for 01-Remove-elan-google-geo-key.php');
                        }
                        ...
                        logProgress('', 'info');
                        logProgress(SECTION_SEPARATOR, 'step');
                        logProgress('Google Geocoding API Key column removal complete', 'step');
                        logProgress(SECTION_SEPARATOR, 'step');
                        ...
                        logProgress('  • Verify the settings table no longer contains elan_google_geo_key', 'info');
                        logProgress('  • Run the #724 fix script to remove elan_google_maps_key', 'info');
                        logProgress('  • Confirm application functionality on the staging environment', 'info');
