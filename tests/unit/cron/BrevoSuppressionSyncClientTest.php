...
/**
 * Unit tests for BrevoSuppressionSyncClient's pre-flight paths (#1923).
 *
 ...
 *
 * Every path here must return null *and* log. Null is the client's only signal
 * that a poll failed, and callers page on `getCount()` and must stop on null
 * rather than conclude the suppression list has ended (see the class
 * docblock) — so a silent null is the one outcome that must never happen.
 */
#[Group('fast')]
final class BrevoSuppressionSyncClientTest extends TestCase
{
    ...
    private function fetch(SendinblueSettingsFakeDatabase $db): ?\Brevo\Client\Model\GetTransacBlockedContacts
    {
        return (new BrevoSuppressionSyncClient($db))->fetchBlockedContacts(
    ...
    // fetchBlockedContacts(). The Brevo SDK is vendored in this working tree
    // and its classes autoload, so getting past apiKey() and loadSdk() reaches
    // the real `new GuzzleHttp\Client()` and makes a live HTTPS call to
    // api.brevo.com. A unit test must not depend on the network, so every case
    // here is one that returns before the HTTP call — and for the same reason
    // the catch-all failure log in fetchBlockedContacts() is not unit-tested
    // either, since reaching it requires making that call.
    //
    // loadSdk()'s branches (autoloader absent, autoloader corrupt, API class
    // still undefined after the require) are consequently not unit-testable
    // either: which one is taken depends on whether a real file exists at a
    // path fixed by a private class constant, and the "SDK present" branch
    // falls through to that live call. They are single log-and-return-false
    // statements, left to code review and manual verification rather than
    // reshaping the class to make a constant injectable purely for a test. The
    // integration suite covers the job end to end with the client substituted.
}
