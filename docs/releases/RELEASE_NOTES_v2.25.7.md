...
- **Pre-merge hardening** (PR #1187): Several silent failures and correctness issues surfaced during pre-merge review: `CarAdministrationService` audit trail now throws `CarDatabaseException` instead of silently returning `true` on failure; `operation_failed` catch-all reclassified from `CarTransferException` (HTTP 409) to `CarDatabaseException` (HTTP 500); `process-car-details.php` catch widened from `Exception` to `\Throwable` to prevent PHP `\Error` subclasses bypassing the `ApiResponse` JSON format; `CarRepository::distinctCarModelValues()` now logs DB errors instead of returning an empty array silently.
