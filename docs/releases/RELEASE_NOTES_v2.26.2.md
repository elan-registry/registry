...
- **PHPStan coverage extended to `app/` and `usersc/includes/`** ([#1317](https://github.com/unibrain1/elanregistry/issues/1317)): Second config `phpstan-app.neon` runs level 2 analysis over all API endpoints, admin scripts, action handlers, and page-level includes (previously only `usersc/classes/` was analysed). Pre-existing errors captured in `phpstan-baseline-app.neon` — CI enforces baseline hygiene via `reportUnmatchedIgnoredErrors: true`, so fixed errors must be dropped from the baseline. See CODING_STANDARDS.md for the fix-when-you-touch-it workflow.
